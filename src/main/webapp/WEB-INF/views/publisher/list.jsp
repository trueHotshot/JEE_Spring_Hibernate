<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%--
  Created by IntelliJ IDEA.
  User: hotshot
  Date: 04.09.18
  Time: 23:24
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
<head>
    <title>List of publishers</title><link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css" integrity="sha384-MCw98/SFnGE8fJT3GXwEOngsV7Zt27NXFoaoApmYm81iuXoPkFOJwJ8ERdknLPMO" crossorigin="anonymous">

</head>
<body>
<div id="wrapper">

    <div class="container">
        <img class="rounded mx-auto d-block mt-5" src="data:image/png;base64,">
        <div class="btn-group btn-group-toggle mx-auto mt-5" data-toggle="buttons">
            <a class="btn btn-primary" href="/books">Books</a> <a class="btn btn-primary" href="/propositions">Propositions</a> <a class="btn btn-primary" href="/authors">Authors</a> <a class="btn btn-secondary active">Publishers</a>
        </div>
        <div class="card card-register mx-auto mt-5">
            <div class="card-header">List of publishers</div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-hover" id="dataTable" width="100%" cellspacing="0">
                        <thead>
                        <tr>
                            <th>Name</th>
                            <th>Books</th>
                            <th>Action</th>
                        </tr>
                        </thead>
                        <tbody>
                        <c:forEach items="${publishers}" var="publisher">
                            <tr>
                                <td><c:out value="${publisher.name}" /></td>
                                <td>books will be here</td>
                                <td>
                                    <a class="btn btn-primary" href="/publishers/form?id=${publisher.id}">Edit</a> <a class="btn btn-primary" href="/publishers/delete/${publisher.id}">Delete</a></td>
                            </tr>
                        </c:forEach>
                        </tbody>
                    </table>
                </div>
                <a class="btn btn-primary" href="/publishers/form">Add Publisher</a>
            </div>
        </div>
    </div>
</div>


<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/js/bootstrap.min.js" integrity="sha384-ChfqqxuZUCnJSK3+MXmPNIyE6ZbWh2IMqE241rYiqJxyMiZ6OW/JmZQ5stwEULTy" crossorigin="anonymous"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.3/umd/popper.min.js" integrity="sha384-ZMP7rVo3mIykV+2+9J3UJ46jBk0WLaUAdn689aCwoqbBJiSnjAK/l8WvCWPIPm49" crossorigin="anonymous"></script>

</body>
</html>

<%@ taglib prefix="form"
           uri="http://www.springframework.org/tags/form" %>
<%--
  Created by IntelliJ IDEA.
  User: hotshot
  Date: 04.09.18
  Time: 11:30
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>

<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<html>
<head>
    <title>Book form</title>
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css" integrity="sha384-MCw98/SFnGE8fJT3GXwEOngsV7Zt27NXFoaoApmYm81iuXoPkFOJwJ8ERdknLPMO" crossorigin="anonymous">

</head>
<body>
<div id="wrapper">

    <div class="container">
        <img class="rounded mx-auto d-block mt-5" src="data:image/png;base64,">
        <div class="card card-register mx-auto mt-5">
            <div class="card-header">Book form</div>
            <div class="card-body">
                <form:form modelAttribute="book" method="post" action="/books/form" id="contact_form">
                    <form:hidden path="id" />
                    <form:hidden path="proposition" />
                    <div class="form-group row">
                        <label class="col-md-2 control-label">Title:</label>
                        <div class="col-md-10 inputGroupContainer">
                            <form:input path="title" cssClass="form-control" cssErrorClass="form-control is-invalid" />
                        </div>
                    </div>
                    <form:errors path="title" cssClass="text-danger form-group row" element="div" />

                    <div class="form-group row">
                        <label class="col-md-2 control-label">Authors:</label>
                        <div class="col-md-10 inputGroupContainer">
                            <form:select path="authors" items="${authors}" itemValue="id" itemLabel="fullName" multiple="true" cssClass="form-control" cssErrorClass="form-control is-invalid" />
                        </div>
                    </div>
                    <form:errors path="authors" cssClass="text-danger form-group row" element="div" />

                    <div class="form-group row">
                        <label class="col-md-2 control-label">Rating:</label>
                        <div class="col-md-10 inputGroupContainer">
                            <form:input path="rating" cssClass="form-control" cssErrorClass="form-control is-invalid" />
                        </div>
                    </div>
                    <form:errors path="rating" cssClass="text-danger form-group row" element="div" />

                    <div class="form-group row">
                        <label class="col-md-2 control-label">Publisher:</label>
                        <div class="col-md-10 inputGroupContainer">
                            <form:select path="publisher.id" cssClass="form-control" cssErrorClass="form-control is-invalid">
                                <form:option value="" label="-- select --" />
                                <form:options items="${publishers}" itemLabel="name" itemValue="id" cssErrorClass="is-invalid" />
                            </form:select>
                        </div>
                    </div>
                    <form:errors path="publisher.id" cssClass="text-danger form-group row" element="div"/>

                    <div class="form-group row">
                        <label class="col-md-2 control-label">Description:</label>
                        <div class="col-md-10 inputGroupContainer">
                            <form:textarea path="description" rows="8" cols="90" cssClass="form-control" cssErrorClass="form-control is-invalid" />
                        </div>
                    </div>
                    <form:errors path="description" cssClass="text-danger form-group row" element="div"/>
                    <div class="col-md-12">
                        <button type="submit" id="submit" class="btn btn-primary btn-block" >Submit</button>
                    </div>
                </form:form>
            </div>
        </div>
    </div>
</div>

<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/js/bootstrap.min.js" integrity="sha384-ChfqqxuZUCnJSK3+MXmPNIyE6ZbWh2IMqE241rYiqJxyMiZ6OW/JmZQ5stwEULTy" crossorigin="anonymous"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.3/umd/popper.min.js" integrity="sha384-ZMP7rVo3mIykV+2+9J3UJ46jBk0WLaUAdn689aCwoqbBJiSnjAK/l8WvCWPIPm49" crossorigin="anonymous"></script>

</body>
</html>
